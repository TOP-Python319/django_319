...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # Получаем разрешение add_card
        add_card_permission = Permission.objects.get(codename='add_card', content_type=content_type)
        # Получаем разрешение delete_card
        delete_card_permission = Permission.objects.get(codename='delete_card', content_type=content_type)
        # Получаем разрешение view_card
        view_card_permission = Permission.objects.get(codename='view_card', content_type=content_type)
        # Добавляем разрешение группе "Moderators"
        moderators_group.permissions.add(change_card_permission, add_card_permission, delete_card_permission, view_card_permission)
