...
- добавил меню в профиле пользователя
...
**commit: `lesson_66: добавили линк в административную панель в профиле модераторов и администраторов`**
...
- 
**commit: `lesson_66: о тестировании в Django`**
...
**commit: `lesson_66: виды тестирования`**

### Суть Test-Driven Development (TDD)

#### Введение
Test-Driven Development (TDD) — это методология разработки программного обеспечения, которая предполагает написание тестов до написания самого кода. TDD помогает разработчикам создавать более качественный и надежный код, минимизируя количество ошибок и упрощая процесс рефакторинга. В этой лекции мы рассмотрим суть TDD, ее основные принципы и преимущества.

#### Суть Test-Driven Development
TDD — это циклический процесс, который включает три основных этапа:

1. **Написание теста (Red)**
   - **Описание**: На первом этапе разработчик пишет тест для функции или метода, который еще не существует. Тест должен проверять поведение, которое ожидается от кода.
   - **Цель**: Определить требования и ожидания от кода до его написания.
   - **Пример**: Написание теста для функции, которая должна возвращать сумму двух чисел.
  
2. **Написание кода (Green)**
   - **Описание**: На втором этапе разработчик пишет минимальный код, который позволяет тесту пройти успешно. Код может быть неоптимальным и даже "грязным", главное — чтобы тест прошел.
   - **Цель**: Обеспечить работоспособность кода и соответствие его требованиям.
   - **Пример**: Написание функции, которая возвращает сумму двух чисел, чтобы тест прошел.

3. **Рефакторинг (Refactor)**
   - **Описание**: На третьем этапе разработчик улучшает и оптимизирует код, не изменяя его поведение. Тесты продолжают проходить успешно.
   - **Цель**: Улучшить качество и читаемость кода, сохраняя его функциональность.
   - **Пример**: Оптимизация функции, которая возвращает сумму двух чисел, чтобы она была более эффективной и читаемой.
  
#### Основные принципы TDD
1. **Приоритет тестов**: Тесты пишутся до кода, что позволяет четко определить требования и ожидания.
2. **Минимальный код**: Код пишется минимально необходимым для прохождения тестов, что помогает избежать излишней сложности.
3. **Постоянный рефакторинг**: Код постоянно улучшается и оптимизируется, что способствует его высокому качеству.
4. **Автоматизация**: Тесты должны быть автоматизированы, чтобы их можно было быстро и легко запускать.

#### Преимущества TDD
1. **Повышение качества кода**: TDD помогает выявлять и исправлять ошибки на ранних стадиях разработки, что значительно снижает затраты на их исправление.
2. **Упрощение рефакторинга**: Тесты обеспечивают уверенность в том, что изменения в коде не нарушат его работоспособность.
3. **Документация**: Тесты служат дополнительной документацией, которая помогает новым разработчикам быстрее понять, как работает система.
4. **Снижение рисков**: TDD помогает минимизировать риски, связанные с внедрением новых изменений в код.
5. **Ускорение разработки**: Хотя написание тестов может потребовать дополнительного времени, в долгосрочной перспективе TDD ускоряет процесс разработки за счет снижения количества ошибок и упрощения рефакторинга.
...
- создали приложение `chat`
- добавили его в `settings.py`
- создали модель данных чата
- создали миграции
- применили миграции
- создали форму сообщения в `forms.py`
- создали представление чата в `views.py`
- создали шаблон для представления чата в `templates/chat/`
- прописали маршруты в `urls.py` приложения `chat`
- прописали маршруты в `urls.py` всего проекта

**commit: `lesson_70: создали простейший чат`**

- дополнили модель данных полями `sender`, `recipient` и `is_broadcast` (для сообщения всем пользователям)
- создали миграции
- применили миграции
- поменяли представление для чата
- поменяли маршруты в `urls.py` приложения `chat`
